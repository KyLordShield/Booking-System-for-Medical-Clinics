...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check for duplicate
    $dupStmt = $conn->prepare("SELECT SPEC_ID FROM specialization WHERE SPEC_NAME = ? AND SPEC_ID != ?");
    $dupStmt->execute([$specName, $specId ?: 0]);
    if ($dupStmt->fetch(PDO::FETCH_ASSOC)) {
        $_SESSION['spec_error'] = "Specialization name already exists!";
    ...
    if ($specId == "") {
        ...
        $_SESSION['spec_success'] = "Specialization added successfully!";
    } else {
        $stmt = $conn->prepare("UPDATE specialization
                                SET SPEC_NAME=?, SPEC_UPDATED_AT=NOW()
                                WHERE SPEC_ID=?");
        $stmt->execute([$specName, $specId]);
        $_SESSION['spec_success'] = "Specialization updated successfully!";
    }
...
if (isset($_GET['delete'])) {
    ...
    $stmt->execute([$id]);
    $_SESSION['spec_success'] = "Specialization deleted successfully!";
    header("Location: specialization.php");
    exit;
}
...
<input type="text" name="q" placeholder="Search specialization..." value="<?= esc($search) ?>" class="px-4 py-2 rounded-full w-60 border focus:outline-none" />
<button class="bg-[var(--primary)] text-white px-5 py-2 rounded-full hover:opacity-90 transition">Search</button>
<?php if ($search !== ""): ?>
<a href="specialization.php" class="bg-gray-400 text-white px-5 py-2 rounded-full hover:opacity-80 transition">Reset</a>
<?php endif; ?>
</form>

<button onclick="openAddModal()" class="bg-[var(--primary)] text-white px-6 py-2 rounded-full hover:opacity-90">
+ Add Specialization
</button>
</div>
...
<tr><td colspan="3" class="text-center py-4 opacity-60">No data found</td></tr>
<?php endif; ?>
...
<td class="py-3 px-4">
<button class="btn bg-[var(--primary)] text-white px-6 py-2 rounded-full ml-2" onclick='openEditModal(<?= json_encode($s) ?>)'>Edit</button>
<button class="btn bg-[var(--primary)] text-white px-6 py-2 rounded-full ml-2" onclick="browseDoctors(<?= $s['SPEC_ID'] ?>, '<?= esc($s['SPEC_NAME']) ?>')">Browse Doctors</button>
</td>
</tr>
<?php endforeach ?>
</tbody>
...
<input id="SPEC_NAME" name="SPEC_NAME" required class="border rounded-lg px-4 py-2 w-full mb-4">
...
async function browseDoctors(specId, specName) {
    ...
    document.getElementById("doctorModalTitle").innerText = "Doctors: " + specName;
    let html = "";
    if (doctors.length === 0) html = `<tr><td colspan="2" class='text-center py-3 opacity-70'>No doctors found</td></tr>`;
...
function closeDoctorsModal() { document.getElementById("doctorModal").style.display = "none"; }

// ✅ Show SweetAlert2 notifications
<?php if(isset($_SESSION['spec_error'])): ?>
Swal.fire({icon:'error',title:'Oops!',text:'<?= $_SESSION['spec_error'] ?>'});
<?php unset($_SESSION['spec_error']); endif; ?>
<?php if(isset($_SESSION['spec_success'])): ?>
Swal.fire({icon:'success',title:'Success',text:'<?= $_SESSION['spec_success'] ?>'});
<?php unset($_SESSION['spec_success']); endif; ?>
</script>
